...
export default function Navigation() {
  ...
  return (
    <nav
      ref={navRef}
      className={`fixed left-0 right-0 z-[100] md:px-6 h-16 transition-all duration-300 ${
        mobileOpen ? 'top-0' : 'top-0 md:top-5'
      }`}
    >
      <div
        className={`mx-auto flex h-full max-w-[1300px] items-center justify-between px-6 md:px-6 transition-all duration-300 ${
            mobileOpen 
              ? 'rounded-none' 
        ...
        {/* Mobile Hamburger */}
        <button
          className="text-[#000] md:hidden relative z-10"
      ...
      <div
        ref={mobileMenuRef}
        className={`absolute left-0 right-0 top-16 border-t border-[rgba(118,76,250,0.15)] bg-[rgba(235,230,255,0.8)] backdrop-blur-md px-6 overflow-hidden transition-all duration-300 ease-in-out md:hidden ${
          mobileOpen 
      ...
      >
        {navLinks.map((link) => (
          <a
            ...
            href={link.href}
            onClick={(e) => handleNavClick(e, link.href)}
            className="block py-3 text-[13px] font-medium uppercase tracking-[0.08em] text-[#1a1a2e] transition-colors hover:text-[#764cfa]"
          >
            {link.label}
          </a>
        ))}
        <a
          href="#cta"
          onClick={(e) => handleNavClick(e, '#cta')}
          className="mt-3 block rounded-full bg-[#764cfa] px-5 py-3 text-center text-[13px] font-medium uppercase tracking-[0.08em] text-white transition-all duration-300 hover:bg-[#a78bfa]"
        >
          Предрегистрация
        </a>
      </div>
